/**
 * IRONCORE - Static Cloud Demo & Fallback Mock Data Engine
...
var MockDB = window.MockDB || {
  ...
  handle(endpoint, options = {}) {
    ...
    if (pathname.includes('/auth/session.php')) {
      const user = this.getStorage('session_user', {
        id: 1,
        name: 'Administrator',
    ...
    if (pathname.includes('/workouts/list.php') || pathname.includes('/workouts/get.php')) {
      const ex = this.getStorage('exercises', []);
      return {
        success: true,
        data: {
